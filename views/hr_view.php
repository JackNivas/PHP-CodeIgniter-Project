...
<script>
   ...
   $(document).ready(function () {
      ...
      $('#create-new-product').click(function () {
            ...
            $('#productCrudModal').html("Add New HR");  //set H4 heading to "Add New Product" of the Modal while pop up
         ...
         submitHandler: function (form)
         {
            ...
            $.ajax({
               ...
               + res.data.name + '</td><td>' + res.data.email + '</td><td>' + res.data.role + '</td><td>'
                + res.data.status + '</td><td><img class="img-profile rounded-circle" style="width:20%; height:20%;" src="<?php echo base_url('uploads/'); ?>"/>' + res.data.profpic +  '</td><td>' + res.data.datecreateon + '</td>';
               ...
               product += '<td><a href="javascript:void(0)" id="" data-id="' + res.data.id + '" class="btn btn-info edit-product"><i class="bi bi-pencil-square"></i></a><a href="javascript:void(0)" id="" data-id="' + 				res.data.id + '" class="btn btn-danger delete-product"><i class="bi bi-trash3"></i></a></td></tr>';
                 ...
                 if (actionType == "create-new-product") {
                     ...
                     logUserAction('Edited Record with ID: ' + res.data.id, 'HR Menu', 'Details of the edited record: ' + res.data.name + ', ' + res.data.email + ', ' + res.data.role + ', ' + res.data.status);
 ...
 $.ajax({
    ...
    success: function (res)
    {
       if (res.success == true) {
          ...
          $('#userCrudModal').html("View HR Details");//heading changed to Edit Product of the Modal
          $('#profpic').html('<img class="img-profile rounded-circle" style="width:20%; height:20%;" src="http://localhost/codeigniter/dashboard/uploads/'+res.data.profpic+'"/>');
         ...
         $.ajax({
            ...
            success: function (res)
            {
               if (res.success == true) {
                  ...
                  $('#productCrudModal').html("Edit HR Details");//heading changed to Edit user of the Modal
         ...
         if (confirm("Are you sure want to delete ID:" +product_id))
         {
            $.ajax({
               ...
               success: function (data) {
                  ...
                  logUserAction('Deleted the Record ID: ' + product_id, 'HR Menu', 'Details of the deleted record: ' + product_id);
